...
void filter_(string filepath, string outlier)
{
...
void sol_average(string filepath, string file_filter, string ouput)
{
    ...
    while (compare(current_time, end_time)) // mỗi lần lặp là tăng thời gian lên 1h 18:00 -> 19:00 -> 1:00
    {
        ...
        for (const auto &entry : id_value)
        {
            ...
            log_file << average << " : " << roundToOneDecimal(average) << endl;
            filter << entry.first << "," << setfill('0')
                   ...
                   << "," << fixed << setprecision(1) << roundToOneDecimal(average) << task_2_2(roundToOneDecimal(average)) << endl;
        }
...
int main()
{
    ...
    for (int i = 1; i < 6; i++)
    {
        ...
        filter_(input, input2);
        sol_average(input, "task2_2.csv", input3);
    }
}